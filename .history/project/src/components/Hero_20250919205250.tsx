...
const Hero: React.FC<HeroProps> = ({ onStartQuiz }) => {
  ...
  // Process hero image to remove near-black background
  const [heroSrc, setHeroSrc] = useState<string>('/hero-beagle.jpg');
  useEffect(() => {
    const img = new Image();
    img.src = '/hero-beagle.jpg';
    img.onload = () => {
      ...
      for (let i = 0; i < d.length; i += 4) {
        ...
        const g = d[i + 1];
        const b = d[i + 2];
        // Remove pure/near-black; soften very dark grays to avoid halo
        if (r < 18 && g < 18 && b < 18) {
          d[i + 3] = 0; // transparent
        } else if (r < 40 && g < 40 && b < 40) {
          d[i + 3] = Math.floor(d[i + 3] * 0.5);
        }
      }
      ctx.putImageData(imageData, 0, 0);
      setHeroSrc(canvas.toDataURL('image/png'));
    };
  }, []);
  return (
